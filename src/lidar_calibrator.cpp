...
// }
	...
	void sensorCallback(const sensor_msgs::ImageConstPtr& image_msg, const sensor_msgs::PointCloud2::ConstPtr& pc_msg) {
		...
		if (coefficients) {
			...
			cv::Point3f img_norm;
			if (imageCheckerBoardDetection(image, img_norm)) {
				std::cout << "PC Norm: " << pc_norm << std::endl;
				std::cout << "IMG Norm: " << img_norm << std::endl;
			}
	...
	bool imageCheckerBoardDetection(cv::Mat& image, cv::Point3f& img_norm) {
		...
		if (patternfound) {
			...
			// construct the object in 3D space
			...
			try {
				...
				cv::Mat z_dir = (cv::Mat_<double>(3, 1) << 0, 0, 1);
				...
				drawAxis(image, corners, ppoints);
				cv::Mat norm = rmat * z_dir;
				// std::cout << "Image Norms" << norm << std::endl;
				img_norm.x = norm.at<double>(0, 0);
				img_norm.y = norm.at<double>(1, 0);
				img_norm.z = norm.at<double>(2, 0);
	...
	std::vector<cv::Point2f> getNormFromPojectedPoints(
	    const std::vector<cv::Point2f>& corners,
	    const std::vector<cv::Point2d>& ppoints) {
		std::vector<cv::Point2f> norm;
		for (int i = 0; i < ppoints.size(); i++) {
			cv::Point2f p;
			p.x = ppoints[i].x - corners[0].x;
			p.y = ppoints[i].y - corners[0].y;
			double n = cv::norm(p);
			p.x /= n;
			p.y /= n;
			norm.push_back(p);
	 ...
	 * @param[in]  corners  The corners
	 * @param[in]  ppoints  The ppoints
	 */
	void drawAxis(cv::Mat& image, const std::vector<cv::Point2f>& corners, const std::vector<cv::Point2d>& ppoints) {
		cv::line(image, corners[0], ppoints[0], cv::Scalar(255, 0, 0), 5);
		cv::line(image, corners[0], ppoints[1], cv::Scalar(0, 255, 0), 5);
		cv::line(image, corners[0], ppoints[2], cv::Scalar(0, 0, 255), 5);
	}
